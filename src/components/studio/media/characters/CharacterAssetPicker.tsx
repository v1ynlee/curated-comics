...
const CHARACTER_ASSET_TYPES = new Set(['character_image', 'gallery_image', 'thumbnail']);
...
export function CharacterAssetPicker({
  ...
  const filtered = useMemo(() => {
    ...
    return assets
      .filter((asset) => !asset.archived && asset.previewUrl && CHARACTER_ASSET_TYPES.has(asset.assetType))
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-bg-surface/35 p-4">
      <label className="font-heading text-sm font-semibold text-text-primary" htmlFor="character-asset-search">Asset Library</label>
      ...
      {filtered.length === 0 && <p className="mt-3 font-body text-sm text-text-secondary">No reusable character assets match this search.</p>}
    </div>
  );
}
